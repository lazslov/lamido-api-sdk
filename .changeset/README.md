...
| `updateInternalDependencies` | `minor` | A patch to `@lamido/api-core` — the HMAC-verifier-fix scenario — reaches consumers through the caret range on its own, without re-releasing three service packages. A **minor** core bump does re-release them, because `^0.1.0` does not admit `0.2.0`. |
...
`@lamido/api-core` is depended on as `workspace:^`, which publishes as `^<version>`. Never pin it:
