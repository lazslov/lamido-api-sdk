/**
 * The email webhook route handler.
 *
 * @remarks
...
/** What {@link createEmailWebhookHandler} accepts. */
export interface EmailWebhookHandlerOptions {
  /**
   * The signing secret, used **whole** — the `whsec_` prefix is key material, not a label to strip.
   *
   * @remarks
   * Defaults to `EMAIL_SERVICE_WEBHOOK_SECRET`, read **per request** rather than at construction, so a
   ...
   * on your `200`. Without this, an order is confirmed twice. Back it with a unique constraint on the
   * event id in your own database — not an in-memory set, which is empty again on the next cold start.
   */
   ...
   *
   * @remarks
   * The response must be a `2xx` within 5 seconds. Five consecutive dead-letters disable your endpoint
   * entirely, and a disabled endpoint has no backlog — nothing arrives later to say what you missed. So
   * anything slower than a queue write belongs on the queue, not here.
   *
   * A throw answers `500` and leaves the event unmarked, so the sender retries — on a ladder bounded by
   * a once-a-day cron, so the retry may be tomorrow. Ordering across events is **not guaranteed**:
   * branch on `data.status` in the payload and on your own stored state, never on arrival order. And an
   * event type you do not recognise arrives here too: ignore it and return.
   */
  readonly onEvent: (event: EmailWebhookEvent) => Promise<void>;
  /**
...
/**
 * Build the `POST` handler for an email webhook route.
 *
 ...
 * | `200` | enqueued | The normal path — including for an event type this SDK does not know. |
 ...
 * is per HTTP attempt and would let every retry through as new. When the header is absent the payload's
 * own `event_id` is used — it is the same value, because the payload is frozen at emission and delivered
 * verbatim afterwards.
 *
 * @example
 * ```ts
...
export function createEmailWebhookHandler(
  options: EmailWebhookHandlerOptions,
): (request: Request) => Promise<Response> {
  const slowAfterMs = options.slowEventWarningMs ?? defaultSlowEventWarningMs;
  let warned = false;

  return async function handleEmailWebhook(request: Request): Promise<Response> {
    ...
    const verdict = await verifyEmailWebhook({ secret, rawBody, headers: request.headers });
    ...
    try {
      ...
      console.error("[@lazslov/email] onEvent threw; the delivery will be retried:", error);
  ...
  function warnIfSlow(elapsedMs: number): void {
    ...
    warned = true;
    console.warn(
      `[@lazslov/email] onEvent took ${Math.round(elapsedMs)}ms. The service treats a response slower ` +
        "than 5 seconds as a failed attempt, and five consecutive dead-letters disable the endpoint " +
        "with no backlog to drain afterwards. Enqueue the work instead of doing it here.",
    );
  }
}
