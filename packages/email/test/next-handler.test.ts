...
import { type EmailWebhookEvent, isMessageEvent } from "../src/webhook.js";
 ...
 *
 * @remarks
 * Delivery is at-least-once and unordered, so every case here is about the ordering: dedupe, then
 * work, then mark. A crash anywhere in that sequence must produce a redelivery rather than a silently
 * dropped event or a doubled confirmation.
 */
...
beforeEach(() => {
  savedSecret = process.env.EMAIL_SERVICE_WEBHOOK_SECRET;
  savedNodeEnv = process.env.NODE_ENV;
  delete process.env.EMAIL_SERVICE_WEBHOOK_SECRET;
});

afterEach(() => {
  if (savedSecret === undefined) delete process.env.EMAIL_SERVICE_WEBHOOK_SECRET;
  else process.env.EMAIL_SERVICE_WEBHOOK_SECRET = savedSecret;
...
function harness(
  overrides: Partial<Parameters<typeof createEmailWebhookHandler>[0]> = {},
  seed: string[] = [],
) {
  const processed = new Set(seed);
  const handled: EmailWebhookEvent[] = [];
...
describe("a valid, first-time delivery", () => {
  it("enqueues, marks processed, and answers 200", async () => {
    ...
    expect(first && isMessageEvent(first) && first.data.public_id).toBe(
      "0194c7a1-0000-7000-8000-000000000002",
    );
    expect([...processed]).toEqual(["0194c7a1-8f3e-7a2b-9c4d-1e5f6a7b8c9d"]);
  });
  ...
  it("reads the secret from the environment when none is passed", async () => {
    process.env.EMAIL_SERVICE_WEBHOOK_SECRET = testWebhookSecret;
    const response = await createEmailWebhookHandler({
  ...
  it("hands an unknown event type to onEvent and still answers 200", async () => {
    // Receiver checklist item 5: ignore an unrecognised event_type and still answer 2xx. That is
    // what makes a new event type additive.
    const { handler, handled } = harness();
    const response = await handler(
      eventRequest({ body: eventBody({ event_type: "message.deferred" }) }),
    );

    expect(response.status).toBe(200);
    expect(handled[0]?.event_type).toBe("message.deferred");
  });
});
...
describe("the dedupe, which is the point of the signature", () => {
  it("answers 200 for a duplicate without calling onEvent", async () => {
    ...
    // produced it.
    const { handler, handled } = harness({}, ["0194c7a1-8f3e-7a2b-9c4d-1e5f6a7b8c9d"]);
    const response = await handler(eventRequest());
  ...
  it("does not dedupe on X-Delivery-Id, which is per attempt", async () => {
    // Deduping on the delivery id would let every retry through as new.
    const seen: string[] = [];
    const { handler } = harness({ alreadyProcessed: async (id) => !!seen.push(id) && false });
  ...
  it("falls back to the payload's own event_id when the header is absent", async () => {
    ...
    expect(seen).toEqual(["0194c7a1-8f3e-7a2b-9c4d-1e5f6a7b8c9d"]);
  });
  ...
  it("processes the second delivery of a DIFFERENT event", async () => {
    ...
    await handler(eventRequest());
    await handler(
      eventRequest({ body: eventBody({ event_id: "0194c7a1-8f3e-7a2b-9c4d-00000000000f" }) }),
    );
...
describe("a body that verified but is not an event", () => {
  it("answers 400, because retrying will not change it", async () => {
    const { handler, handled } = harness();
    const response = await handler(eventRequest({ body: '{"event_type":"message.sent"}' }));
...
describe("an unconfigured deployment", () => {
  it("answers 500 naming the variable, rather than throwing on import", async () => {
    ...
    expect(response.status).toBe(500);
    expect(await response.text()).toMatch(/EMAIL_SERVICE_WEBHOOK_SECRET/);
  });
});
