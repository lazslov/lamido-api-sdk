...
/**
 * The single most important design decision in phase 6, asserted at the type level: **the handler
 * cannot be constructed without the dedupe.**
 *
 * @remarks
 * Delivery is at-least-once, and the SDK owns no storage — so it cannot dedupe on a consumer's behalf.
 * What it can do is make forgetting it a compile error rather than a doubled fulfilment discovered from
 * a customer's second charge.
 *
...
describe("the dedupe callbacks are required", () => {
  ...
  it("cannot be constructed without onEvent", () => {
    ...
    const call = () => createPaymentWebhookHandler({ alreadyProcessed, markProcessed });
    expect(typeof call).toBe("function");
  });

  it("is constructible with all three, and the secret stays optional", () => {
    const handler = createPaymentWebhookHandler({ alreadyProcessed, markProcessed, onEvent });
...
describe("the handler is a plain Web handler", () => {
  it("takes a Request and answers a Response, with no framework type in the signature", () => {
    ...
    const handler: (request: Request) => Promise<Response> = createPaymentWebhookHandler({
      alreadyProcessed,
      markProcessed,
      onEvent,
    });
