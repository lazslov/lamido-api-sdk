/**
 * The webshop webhook route handler.
 *
 * @remarks
 * Nothing in this module imports `next`. It takes a `Request` and answers a `Response`, which is what
 * an App Router route handler is — so it works unchanged in any Web-standard runtime. It lives on the
 * `./next` subpath because that is where a consumer looks for it, and because the *documentation*
 * around it is Next-specific: `export const runtime = "nodejs"` is the line that keeps the HMAC valid.
 *
...
/** What {@link createWebshopWebhookHandler} accepts. */
export interface WebshopWebhookHandlerOptions {
  /**
   * The signing secret, used **whole** — the `whsec_` prefix is key material, not a label to strip.
   *
   * @remarks
   * Defaults to `WEBSHOP_WEBHOOK_SECRET`, read **per request** rather than at construction, so a route
   * module cannot throw on import and take the route tree down with it — and so a site still builds
   ...
   *
   * @remarks
   * Delivery is at-least-once: the same event arrives again after a timeout, a `5xx`, a network blip
   * on your `200`, or an operator's redelivery. Without this, fulfilment runs twice. Back it with a
   * unique constraint on the event id in your own database — not an in-memory set, which is empty
   ...
   * five exhausted ladders disable your endpoint entirely — so sending an email, issuing an invoice or
   * updating stock belongs on a queue, not here.
   *
   * A throw answers `500` and leaves the event unmarked, so the sender retries. Ordering across events
   * is **not guaranteed**: branch on `data.order.status` in the payload, never on arrival order — and
   * remember `order.created` is an observation whose status must be read.
   */
  readonly onEvent: (event: WebshopWebhookEvent) => Promise<void>;
  /**
...
/**
 * Build the `POST` handler for a webshop webhook route.
 *
 ...
 * | `200` | enqueued | The normal path, including an event type this SDK does not know. |
 ...
 *
 * The dedupe key is `X-Event-Id`, which is stable across every retry and every redelivery.
 * `X-Delivery-Id` is per HTTP attempt and would let the same event through up to eight times. When
 * the header is absent the payload's own `event_id` is used — it is the same value, because the payload
 ...
 *   onEvent: async (event) => {
 *     if (isKnownEvent(event)) await queue.push({ orderId: event.data.order.public_id });
 *   },
 * });
 * ```
 */
export function createWebshopWebhookHandler(
  options: WebshopWebhookHandlerOptions,
): (request: Request) => Promise<Response> {
  const slowAfterMs = options.slowEventWarningMs ?? defaultSlowEventWarningMs;
  let warned = false;

  return async function handleWebshopWebhook(request: Request): Promise<Response> {
    ...
    const verdict = await verifyWebshopWebhook({ secret, rawBody, headers: request.headers });
    ...
    try {
      ...
      console.error("[@lazslov/webshop] onEvent threw; the delivery will be retried:", error);
  ...
  function warnIfSlow(elapsedMs: number): void {
    ...
    warned = true;
    console.warn(
      `[@lazslov/webshop] onEvent took ${Math.round(elapsedMs)}ms. The service treats a response slower ` +
        "than 5 seconds as a failed attempt; eight of those dead-letter the delivery and five " +
        "exhausted ladders disable the endpoint. Enqueue the work instead of doing it here.",
    );
  }
}
