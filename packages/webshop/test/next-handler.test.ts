...
import { isKnownEvent, type WebshopWebhookEvent } from "../src/webhook.js";
import { eventBody, eventRequest, orderBlock, testWebhookSecret } from "./stubs/delivery.js";
...
beforeEach(() => {
  savedSecret = process.env.WEBSHOP_WEBHOOK_SECRET;
  savedNodeEnv = process.env.NODE_ENV;
  delete process.env.WEBSHOP_WEBHOOK_SECRET;
});

afterEach(() => {
  if (savedSecret === undefined) delete process.env.WEBSHOP_WEBHOOK_SECRET;
  else process.env.WEBSHOP_WEBHOOK_SECRET = savedSecret;
...
function harness(
  overrides: Partial<Parameters<typeof createWebshopWebhookHandler>[0]> = {},
  seed: string[] = [],
) {
  const processed = new Set(seed);
  const handled: WebshopWebhookEvent[] = [];
...
describe("a valid, first-time delivery", () => {
  it("enqueues, marks processed, and answers 200", async () => {
    ...
    expect(first && isKnownEvent(first) && first.data.order.public_id).toBe(
      "019f1c40-0000-7000-8000-0000000000c3",
    );
    expect([...processed]).toEqual(["019f1c40-0000-7000-8000-0000000000a1"]);
  });
  ...
  it("reads the secret from the environment when none is passed", async () => {
    process.env.WEBSHOP_WEBHOOK_SECRET = testWebhookSecret;
    const response = await createWebshopWebhookHandler({
  ...
  it("carries an order.created's status through, for the receiver to read", async () => {
    ...
    expect(event && isKnownEvent(event) && event.data.order.status).toBe("pending");
  });

  it("answers 200 for an event type this SDK does not know, and still hands it over", async () => {
    // Answering non-2xx would dead-letter a delivery that was fine. This is what makes a new type additive.
    const { handler, handled } = harness();
    const response = await handler(
      eventRequest({ body: eventBody({ event_type: "order.disputed" }) }),
    );

    expect(response.status).toBe(200);
    expect(handled[0]?.event_type).toBe("order.disputed");
  });
});
...
describe("the dedupe, which is the point of the signature", () => {
  it("answers 200 for a duplicate without calling onEvent", async () => {
    ...
    // produced it.
    const { handler, handled } = harness({}, ["019f1c40-0000-7000-8000-0000000000a1"]);
    const response = await handler(eventRequest());
  ...
  it("falls back to the payload's own event_id when the header is absent", async () => {
    ...
    expect(seen).toEqual(["019f1c40-0000-7000-8000-0000000000a1"]);
  });
  ...
  it("processes the second delivery of a DIFFERENT event", async () => {
    ...
    await handler(eventRequest());
    await handler(
      eventRequest({ body: eventBody({ event_id: "019f1c40-0000-7000-8000-00000000000f" }) }),
    );
...
describe("a body that verified but is not an event", () => {
  it("answers 400, because retrying will not change it", async () => {
    const { handler, handled } = harness();
    const response = await handler(eventRequest({ body: '{"event_type":"order.invented"}' }));
...
describe("an unconfigured deployment", () => {
  it("answers 500 naming the variable, rather than throwing on import", async () => {
    ...
    expect(response.status).toBe(500);
    expect(await response.text()).toMatch(/WEBSHOP_WEBHOOK_SECRET/);
  });
});
