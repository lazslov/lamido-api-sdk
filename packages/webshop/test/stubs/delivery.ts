/**
 * Signed webshop webhook deliveries, built the way the service builds them.
 *
 * @remarks
...
/** The order block webhooks.md §3 documents, as a mutable object a case can adjust. */
export function orderBlock(overrides: Record<string, unknown> = {}) {
  return {
...
/** An `order.confirmed` body, in the estate's standard event envelope. */
...
export function eventRequest(options: EventOptions = {}): Request {
  ...
  return new Request("https://site.example.com/api/webhooks/webshop", {
    method: "POST",
    headers,
    body,
  });
