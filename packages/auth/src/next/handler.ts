...
/** What {@link createAuthWebhookHandler} accepts. */
export interface AuthWebhookHandlerOptions {
  /**
   * The signing secret, used **whole** — the `whsec_` prefix is key material, not a label to strip.
   *
   * @remarks
   * Defaults to `AUTH_SERVICE_WEBHOOK_SECRET`, read **per request** rather than at construction, so a
   ...
   * on your `200`, and an operator can redeliver it days later under the same `X-Event-Id`. Back this
   * with a unique constraint on the event id in your own database — not an in-memory set, which is
   * empty again on the next cold start.
   */
   ...
   * five consecutive failures disable your endpoint entirely — so provisioning a tenant or revoking
   * access belongs on a queue, not here.
   *
   * A throw answers `500` and leaves the event unmarked, so the sender retries. Ordering across events
   * is **not guaranteed**: reconcile against `occurred_at` and the block's `status`, never against
   * arrival order. A `webhook.ping` and an unrecognised type reach here too — acknowledge them.
   */
  readonly onEvent: (event: AuthWebhookEvent) => Promise<void>;
  /**
...
/**
 * Build the `POST` handler for an auth webhook route.
 *
 ...
 * | `200` | enqueued | The normal path, including a `webhook.ping` and an unrecognised type. |
 ...
 *   onEvent: async (event) => {
 *     await queue.push({ type: event.event_type, tenant: event.tenant.public_id });
 *   },
 * });
 * ```
 */
export function createAuthWebhookHandler(
  options: AuthWebhookHandlerOptions,
): (request: Request) => Promise<Response> {
  const slowAfterMs = options.slowEventWarningMs ?? defaultSlowEventWarningMs;
  let warned = false;

  return async function handleAuthWebhook(request: Request): Promise<Response> {
    ...
    const verdict = await verifyAuthWebhook({ secret, rawBody, headers: request.headers });
    ...
    try {
      ...
      console.error("[@lazslov/auth] onEvent threw; the delivery will be retried:", error);
  ...
  function warnIfSlow(elapsedMs: number): void {
    ...
    warned = true;
    console.warn(
      `[@lazslov/auth] onEvent took ${Math.round(elapsedMs)}ms. The service treats a response slower ` +
        "than 5 seconds as a failed attempt; eight of those dead-letter the delivery and five " +
        "consecutive failures disable the endpoint. Enqueue the work instead of doing it here.",
    );
  }
}
