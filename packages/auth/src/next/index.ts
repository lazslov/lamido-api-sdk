/**
 * `@lazslov/auth/next` — the webhook route handler.
 *
 * @remarks
 ...
 *
 * The whole point of the shape is that {@link createAuthWebhookHandler} **cannot be constructed
 ...
 *   onEvent: (event) => queue.push({ type: event.event_type, tenant: event.tenant.public_id }),
 * });
 * ```
 */

export { type AuthWebhookHandlerOptions, createAuthWebhookHandler } from "./handler.js";
