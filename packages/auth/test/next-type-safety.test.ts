 ...
 * What it can do is make forgetting it a compile error rather than a doubled provisioning discovered
 * from a tenant's second welcome mail.
 *
...
describe("the dedupe callbacks are required", () => {
  ...
  it("cannot be constructed without onEvent", () => {
    ...
    const call = () => createAuthWebhookHandler({ alreadyProcessed, markProcessed });
    expect(typeof call).toBe("function");
  });

  it("is constructible with all three, and the secret stays optional", () => {
    const handler = createAuthWebhookHandler({ alreadyProcessed, markProcessed, onEvent });
...
describe("the handler is a plain Web handler", () => {
  it("takes a Request and answers a Response, with no framework type in the signature", () => {
    ...
    const handler: (request: Request) => Promise<Response> = createAuthWebhookHandler({
      alreadyProcessed,
      markProcessed,
      onEvent,
    });
