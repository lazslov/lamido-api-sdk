...
- **Subpath exports arrive with the phase that builds them.** The plan's §3 example shows
  `./fields` and `./next` on `@lazslov/content`; declaring them before the files exist would
...
Phase 2 is complete: `@lazslov/api-core` exports the eight primitives its plan lists.
...
Phase 3 is complete: `@lazslov/content` ships both consumer tiers, the field-descriptor layer on a
...
- **`getHealth` smuggles a `503` back through the transport's error path.** A private error
  ...
  than the detour.
- **`buildQuery` in `@lazslov/api-core` now serialises an array as a repeated parameter.** Needed by
...
- **`getHealth` returns the degraded `503` body instead of throwing**, which phase 4's plan does not
  ...
  still bites. Same private-error-subclass detour as `@lazslov/content`, for the same reason: the
...
Phase 5 is complete: `@lazslov/payment` ships the seven merchant endpoints, the money type, RFC 7807
...
Phase 6 is complete: `@lazslov/content/next` ships the three cache modes, the revalidation handler and
the server-action error shape; `@lazslov/payment/next` ships the webhook handler.
...
- **Only `@lazslov/content` declares the peer dependency.** `@lazslov/payment/next` imports nothing from
...
- **`examples/node-script` cannot prove "next is not installed".** The first version asserted
  `require("@lazslov/content/next")` throws. It does not: pnpm hoists the repository's own `next`
...
- **`pnpm deps:audit` must exclude declared peers, and must run from the root with `--filter`.** Two
  ...
  devDependency, which dragged fifty packages of Next's tree into `@lazslov/content`'s graph. An optional
...
- **The scope is `@lazslov`, not `@lamido`.** The plan assumed a `@lamido` npm organisation would be
  ...
  `/-/org/lamido/user` answers `{"lamido":"owner"}` — the shape a **user** scope returns, where an
  organisation (`vercel`, `okeoke`) returns `{}` and an unclaimed name 404s — with zero packages
  published, and whether that account is ours is not answerable from this machine (the local npm token
  is a legacy read-limited one that 403s even on `okeoke`, an organisation the account demonstrably
  belongs to). And the maintainer publishes from a personal account. So all four packages were renamed
  to the maintainer's own username scope, which needs no organisation, no membership and no paid plan:
  a free account publishes unlimited *public* packages under its own scope, and none of these are
  private. Renaming cost one mechanical pass over 121 files; doing it after a publish would have cost a
  deprecation of four packages and a migration note.

  `Lamido` stays everywhere it is the **project**: the repository name, `LAMIDO_KB_PATH`, and the prose
  describing whose services these are. Only the npm scope moved.
...
- **`tsconfig.json` maps `@lazslov/api-core` to its source** so `pnpm typecheck` runs from a clean
